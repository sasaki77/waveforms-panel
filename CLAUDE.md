...
- **`src/data/chartData.ts`** — `makeChartData` builds the Chart.js datasets at a given waveform `index` (i.e. a given timestamp column), applying palette color (from the theme it is handed), per-series hidden state and display-mode-driven line/point styling. Each dataset carries a `custom.key` (derived from `refId`/`name`) used to correlate legend clicks back to a series.
