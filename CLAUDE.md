...
- **`src/data/legend.ts`** — `makeLegendItems` / `updateHiddenSeries` implement custom legend click behavior (click to isolate a series, ctrl/cmd-click to toggle) rather than using Chart.js's or Grafana's built-in legend interactivity — `VizLegend` renders, but the click handling is ours. `getItemKey` appends the series position to the key: `VizLegendList` uses it as the React list key and falls back to the *label* without it, and duplicate keys make the rendered list accumulate entries as the slider moves. Grafana's own `PlotLegend` does the same thing, keying by label plus frame and field index. Including the position means nothing upstream has to guarantee uniqueness for the list to keep working.
